...
export default function CreateDelivery() {
   ...
   const handleSubmit = async (data: any, CB: (arg0: { success: boolean; error?: any }) => void) => {
      ...
      const res = await DeliveryServices.createDelivery(value)
      const { success, error, } = res
      if (success) {
         setInitialValues({...data, qty: 0})
         return CB({ success })
      }
      if (!success && error) {
               ...
               <div className="w-full">
                  {delivery?.map((item, i) => (
                     <div className="p-2 border-b border-b-gray-500 flex justify-between align-middle" key={i}>
                        <div className="w-[40%]">
                           <strong><Link to={`/rbs/v2/orders/${item.Order_id._id}`}>{item?.Order_id?.Order_no}</Link></strong>
