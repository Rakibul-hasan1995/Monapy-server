...
export default function UpdateReceive() {
   ...
   useEffect(() => {
      ...
      const data = {
         date: moment(new Date(value[_id]?.Receive_date)).format('yyyy-MM-DD'),
         chalanNo: value[_id]?.Receive_ch_no,
         Client_id: value[_id]?.Client_id._id,
         Order_id: value[_id]?.Order_id._id,
         qty: value[_id]?.Receive_qty
      }
   ...
   const handleSubmit = async (data: any, CB: (arg0: { success: boolean; error?: any }) => void) => {
      ...
      const res = await ReceiveServices.updateReceive(formData)
   ...
   return (
      <Layout title={`Update Receive: ${initialValue.chalanNo}`}>
